...
export class RigidBody2D {
   ...
   */
  constructor(gameObject, hitboxSize = new Vec2(1, 1)) {
   ...
   */
  checkBodyOnBodyCollision(otherBody) {
   ...
   */
  checkCollideScreenEdge(edgeMargin = 1) {
    ...
    if (
      this.gameObject.physicsBody2D.position.x <=
      edgeMargin + this.hitboxSize / 2
    ) {
      collisionDirection = "left";
    } else if (
      this.gameObject.physicsBody2D.position.x >=
      window.innerWidth - edgeMargin - this.hitboxSize / 2
    ) {
      ...
      this.gameObject.physicsBody2D.position.y >=
      window.innerHeight - edgeMargin - this.hitboxSize / 2
    ) {
    ...
    if (collisionDirection) {
      const collisionEvent = new CustomEvent("screenEdgeCollision", {
        detail: {
